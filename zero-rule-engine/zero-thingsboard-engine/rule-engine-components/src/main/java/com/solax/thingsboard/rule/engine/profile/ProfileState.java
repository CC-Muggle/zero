 ...
 */
package com.solax.thingsboard.rule.engine.profile;
...
class ProfileState {
    ...
    private final Map<String, Map<AlarmSeverityEnum, Set<AlarmConditionFilterKey>>> alarmCreateKeys = new HashMap<>();
    ...
    void updateDeviceProfile(DeviceProfile deviceProfile) {
        ...
        if (deviceProfile.getProfileData().getAlarms() != null) {
            ...
            for (DeviceProfileAlarm alarm : deviceProfile.getProfileData().getAlarms()) {
                Map<AlarmSeverityEnum, Set<AlarmConditionFilterKey>> createAlarmKeys = alarmCreateKeys.computeIfAbsent(alarm.getId(), id -> new HashMap<>());
    ...
    Set<AlarmConditionFilterKey> getCreateAlarmKeys(String id, AlarmSeverityEnum severity) {
        Map<AlarmSeverityEnum, Set<AlarmConditionFilterKey>> sKeys = alarmCreateKeys.get(id);
        if (sKeys == null) {
