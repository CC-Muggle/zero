 ...
 */
package com.solax.thingsboard.rule.engine.edge;
...
public class TbMsgPushToEdgeNode extends AbstractTbMsgPushNode<TbMsgPushToEdgeNodeConfiguration, EdgeEvent, EdgeEventType> {
    ...
    @Override
    EdgeEventType getEventTypeByEntityType(EntityTypeEnum entityTypeEnum) {
        return EdgeUtils.getEdgeEventTypeByEntityType(entityTypeEnum);
    }
    ...
    protected void processMsg(TbContext ctx, TbMsg msg) {
        try {
            if (EntityTypeEnum.EDGE.equals(msg.getOriginator().getEntityType())) {
